...
AdjSet::AdjSet(std::string filename) {
	std::ifstream ifs;
...
void AdjSet::_validateVertex(unsigned int v) {
	if (v < 0 || v >= _v) {
...
bool AdjSet::hasEdge(unsigned int v, unsigned int w) {
	_validateVertex(v);
	_validateVertex(w);
...
std::list<unsigned int> AdjSet::adj(unsigned int v) {
	_validateVertex(v);
...
unsigned int AdjSet::degree(int v) {
	_validateVertex(v);

	return _adj[v].size();
};

unsigned int AdjSet::V() {
	return _v;
};

unsigned int AdjSet::E() {
	return _e;
};
